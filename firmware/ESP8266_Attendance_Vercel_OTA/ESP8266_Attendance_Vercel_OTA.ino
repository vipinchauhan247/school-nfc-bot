...
bool parseSuccessResponse(const String& response, String& studentName, String& typeStr,
  ...
  if (p1 < 0 || p2 <= p1 + 1 || p3 <= p2 + 1 || p3 >= (int)response.length() - 1)
    return false;
  studentName = response.substring(p1 + 1, p2);
  typeStr = response.substring(p2 + 1, p3);
  timeStr = response.substring(p3 + 1);
  return studentName.length() > 0 && (typeStr == "IN" || typeStr == "OUT") &&
         timeStr.length() > 0;
}
